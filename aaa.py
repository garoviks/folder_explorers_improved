...
class CustomHandler(SimpleHTTPRequestHandler):
    ...
    def list_directory(self, path):
        ...
        r.append('<style>')
        r.append(
            'body { font-family: "Segoe UI", Roboto, Helvetica, Arial, sans-serif; margin: 2rem; background: #121212; color: #e0e0e0; }')
        r.append('h1 { color: #ffffff; margin: 0; }')
        r.append(
            'table { width: 100%; border-collapse: collapse; background: #1e1e1e; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.3); }')
        ...
        r.append(
            '.count-btn { background-color: #4CAF50; border: none; color: white; padding: 10px 20px; text-align: center; cursor: pointer; border-radius: 4px; }')
        ...
        r.append(
            '.modal-content { background-color: #1e1e1e; margin: 15% auto; padding: 20px; border: 1px solid #888; width: 50%; border-radius: 8px; color: white; text-align: center; }')
        r.append(
            '.cmd-box { background: #eee; padding: 10px; font-family: monospace; overflow-x: auto; white-space: pre-wrap; margin-bottom: 20px; border-left: 5px solid #ccc; color: #333; }')
        r.append(
            '#executionOutput { display: none; margin-top: 30px; background: #1e1e1e; color: #d4d4d4; padding: 15px; font-family: monospace; border-radius: 5px; border: 1px solid #333; }')
        r.append('</style>\n</head>\n<body>')
        ...
        if self.path != '/':
            r.append(
                '<tr><td></td><td><a href="../" class="dir">📁 ../ (Parent Directory)</a></td><td>-</td><td>-</td><td>-</td></tr>')
        ...
        for name in list_dir:
            ...
            r.append(
                f'<tr><td>{chk_box}</td><td><a href="{linkname}" class="{"dir" if is_dir else ""}" onclick="saveFolderScroll(\'{html.escape(name)}\')">{html.escape(displayname)}</a></td><td>{items_str}</td><td>{size_str}</td><td>{mtime}</td></tr>')
        ...
        r.append(
            'function updateSelectedFiles() { document.querySelectorAll(".item-chk").forEach(chk => { if(chk.checked) selectedFiles.add(chk.value); else selectedFiles.delete(chk.value); }); }')
        ...
        r.append(
            '    }).then(res => res.json()).then(data => { if(data.error) alert(data.error); else location.reload(); });')
        r.append('  }')
        r.append('}')

        r.append('function prepareCreateCBZ() {')
        r.append(
            '  let files = getSelectedFiles(); if(files.length < 2) { alert("Select at least 2 files."); return; }')
        r.append(
            '  let outputDiv = document.getElementById("executionOutput"); let contentDiv = document.getElementById("outputContent");')
        r.append('  outputDiv.style.display = "block"; contentDiv.innerText = "Processing...";')
        r.append(
            '  fetch("/", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ abs_path: document.getElementById("currentAbsPath").value, pattern: files[0].split(" ")[0] + "*.*", outname: files[0].split(" ")[0] + " v01" }) })')
        r.append(
            '  .then(res => res.json()).then(data => { contentDiv.innerText = data.output || data.error; setTimeout(() => location.reload(), 3000); });')
        r.append('}')

        r.append(
            'function toggleSelectAll() { let chks = document.querySelectorAll(".item-chk"); let state = !chks[0].checked; chks.forEach(c => c.checked = state); updateSelectedFiles(); }')
