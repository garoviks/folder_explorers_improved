"""
Script: action_explorer_v12.py
Version: v12
Description: A custom HTTP web server that provides an interactive directory listing.
v12 fixes:
- parse_filename (Python) and parseFilename (JS): now also recognise issue numbers
  in parentheses, e.g. "3W3M (001) - Fable (2021)" — previously all files in such
  a series were treated as different series groups, triggering a false multi-group warning.
"""
...
def parse_filename(filename: str) -> dict:
    ...
    series_match = re.match(r'^(.*?)(?:\s+(?:#?\d+|\(\d{1,3}\))(?:\s*\(of\s*\d+\))?)', name, re.IGNORECASE)
    series = series_match.group(1).strip() if series_match else name
    subtitle = None
    subtitle_match = re.search(r'(?:#?\d+|\(\d{1,3}\))(?:\s*\(of\s*\d+\))?\s*-\s*(.+)', name, re.IGNORECASE)
    if subtitle_match:
...
def create_cbz_direct(abs_path: str, files: list[str], outname: str) -> tuple[bool, str]:
    """
    Build a CBZ from exactly the listed files using 7z + zip.
    ...
    try:
        ...
        # Step 2: extract each selected file into its own subfolder
        # Use unrar for .cbr (supports RAR5), unzip for .cbz — same as makecbz.py
        ...
        # Step 3: zip the working folder into a CBZ
        log.append(f'\n[3/6] Zipping into {cbz_name}')
        info(f'Source: {working_dir}')
        info(f'Output: {cbz_in_working}')
        ...
        cbz_size = os.path.getsize(cbz_in_working)
        cbz_size_str = (f'{cbz_size/1024**3:.2f} GB' if cbz_size >= 1024**3
                        else f'{cbz_size/1024**2:.1f} MB' if cbz_size >= 1024**2
                        else f'{cbz_size/1024:.1f} KB')
        ok(f'Created {cbz_name} ({cbz_size_str})')

        # Step 4: move CBZ to abs_path
        log.append(f'\n[4/6] Moving CBZ to destination')
        info(f'From: {cbz_in_working}')
        info(f'To:   {cbz_final}')
        if os.path.exists(cbz_final):
            info(f'Existing file found at destination, removing.')
            os.remove(cbz_final)
...
def check_folder_for_cbz(folder_path: str) -> list[dict]:
    ...
    def scan(dirpath: str, depth: int):
        ...
        if not groups and not comic_files:
            if depth == 2 or not subdirs:
                rows.append({'path': dirpath, 'series': '', 'file_count': 0,
                             'volumes_skipped': len(volumes_skipped), 'status': 'no_comics',
                             'proposed_outname': '', 'has_deeper_subfolders': has_deeper, 'files': ''})
        else:
            for series, files in groups.items():
                ...
                status = 'cbz_exists' if existing_cbz else ('ready' if len(files) >= 2 else 'single_file')
                rows.append({'path': dirpath, 'series': series, 'file_count': len(files),
                             'volumes_skipped': len(volumes_skipped), 'status': status,
                             'proposed_outname': proposed, 'has_deeper_subfolders': has_deeper,
        ...
        if depth < 2:
            ...
            rows.append({'path': dirpath, 'series': '', 'file_count': 0, 'volumes_skipped': 0,
                         'status': 'deeper_only', 'proposed_outname': '',
                         'has_deeper_subfolders': True, 'files': ''})
    ...
    for sub in sorted(e for e in top_entries if os.path.isdir(os.path.join(folder_path, e))):
...
def render_csv_as_html(csv_path: str) -> str:
    ...
    for r in rows:
        ...
        path = r.get('path', '')
        series = r.get('series', '')
        series_path = os.path.join(path, series) if series else ''
        table_rows.append(
            f'<tr data-status="{html.escape(status)}">'
            f'<td style="border-left:4px solid {colour};padding-left:12px">'
            f'{folder_link(path)}</td>'
            f'<td>{html.escape(series)}'
            + (f'<br><a href="{folder_url(series_path)}" target="_blank" '
               f'style="color:#888;font-size:0.78em;font-weight:normal;word-break:break-all">'
               f'{html.escape(series_path)}</a>' if series else '') +
            f'</td>'
...
function applyFilter() {{
  document.querySelectorAll('#csvBody tr').forEach(row => {{
    row.classList.toggle('hidden', currentFilter !== 'all' && row.dataset.status !== currentFilter);
  }});
}}
...
class CustomHandler(SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # --- Create CBZ directly from file list ---
        if action == 'create_cbz_direct':
            files = data.get('files', [])
            outname = data.get('outname', 'output')
            success, log = create_cbz_direct(abs_path, files, outname)
            self.send_response(200)
            self.send_header('Content-type', 'application/json')
            self.end_headers()
            self.wfile.write(json.dumps({'output': log, 'success': success}).encode())
            return
        ...
        if action == 'check_subfolders':
            try:
                ...
                self.send_header('Content-type', 'application/json')
                self.end_headers()
                self.wfile.write(json.dumps({'output': summary, 'csv_path': csv_path, 'view_url': view_url}).encode())
            except Exception as e:
                self.send_response(500)
                self.send_header('Content-type', 'application/json')
                self.end_headers()
    ...
    def list_directory(self, path):
        ...
        r.append('.editname-btn { background-color: #e65100; border: none; color: white; padding: 8px 20px; cursor: pointer; border-radius: 4px; font-size: 0.95em; font-weight: 700; }')
        r.append('.editname-btn:hover { background-color: #ef6c00; }')
        r.append('#outnameEdit { background: #2c2c2c; color: #fff; border: 1px solid #555; border-radius: 4px; padding: 6px 10px; font-size: 0.95em; width: 420px; margin-top: 8px; display: none; }')
        ...
        r.append('#csvActions { display: none; margin-top: 12px; gap: 10px; }')
        ...
        r.append('#dragSelectBox { position: fixed; border: 1px solid #64b5f6; background: rgba(100,181,246,0.12); pointer-events: none; z-index: 9999; display: none; }')
        r.append('</style>\n</head>\n<body>')

        r.append('<div id="topWarning"></div>')
        r.append('<div id="dragSelectBox"></div>')
        ...
        r.append(
            ...
            '<div id="cbzConfirm" style="display:none;margin-top:12px;flex-wrap:wrap;gap:10px;align-items:center">'
            ...
            '<button class="editname-btn" onclick="toggleRenameBox()">✎ Rename</button>'
            '<br><input id="outnameEdit" type="text" placeholder="Edit output name...">'
            '</div>'
            '<div id="csvActions">'
...
function hideWarning() { document.getElementById("topWarning").style.display = "none"; }
function showWarning(msg) {
  let w = document.getElementById("topWarning");
  w.innerText = msg; w.style.display = "block";
}
...
function updateSelectedFiles() {
  ...
  if (skipped > 0) showWarning(skipped + " item(s) skipped — Volume/Collected editions excluded.");
}
...
function toggleSelectAll() {
  const chks = Array.from(document.querySelectorAll(".item-chk"))
                    .filter(c => c.getAttribute("data-isdir") === "false");
  const anyChecked = chks.some(c => c.checked);
  chks.forEach(c => { c.checked = !anyChecked; });
  updateSelectedFiles();
}
...
(function () {
  ...
  // Snapshot of checkbox states when drag begins (so we toggle relative to original)
  ...
  function updateSelection(x1, y1, x2, y2) {
    let selLeft   = Math.min(x1, x2);
    let selTop    = Math.min(y1, y2);
    let selRight  = Math.max(x1, x2);
    let selBottom = Math.max(y1, y2);

    document.querySelectorAll("#csvTable tbody tr, table tbody tr").forEach(row => {
      let chk = row.querySelector(".item-chk");
      if (!chk) return;
      let r = row.getBoundingClientRect();
      let hit = rectsOverlap(selLeft, selTop, selRight, selBottom,
                              r.left, r.top, r.right, r.bottom);
      chk.checked = hit ? true : (baseState.get(chk) || false);
    });
    updateSelectedFiles();
  }

  document.addEventListener("mousedown", function (e) {
    // Only trigger on left-click on bare table rows — not on checkboxes, links, buttons
    if (e.button !== 0) return;
    let tag = e.target.tagName.toLowerCase();
    if (["input","a","button","th"].includes(tag)) return;
    // Must be inside the file table area
    ...
    // Snapshot current checkbox states
    baseState.clear();
    document.querySelectorAll(".item-chk").forEach(chk => {
      baseState.set(chk, chk.checked);
    });
  ...
  document.addEventListener("mousemove", function (e) {
    ...
    updateSelection(startX, startY, curX, curY);

    // Auto-scroll when dragging near viewport edges
    const EDGE = 40, SPEED = 12;
    if (curY < EDGE)                          window.scrollBy(0, -SPEED);
    else if (curY > window.innerHeight - EDGE) window.scrollBy(0,  SPEED);
  });
...
function parseFilename(filename) {
  ...
  let seriesMatch = name.match(/^(.*?)(?:\\s+(?:#?\\d+|\\(\\d{1,3}\\))(?:\\s*\\(of\\s*\\d+\\))?)/i);
  let series = seriesMatch ? seriesMatch[1].trim() : name;
  let subtitleMatch = name.match(/(?:#?\\d+|\\(\\d{1,3}\\))(?:\\s*\\(of\\s*\\d+\\))?\\s*-\\s*(.+)/i);
  let subtitle = null;
  if (subtitleMatch) {
    subtitle = subtitleMatch[1].replace(/\\(\\d{4}\\)/g,'').replace(/\\([^)]+\\)/g,'').replace(/\\s+/g,' ').trim();
    if (!subtitle) subtitle = null;
  }
  return { series, subtitle, year };
}

function getNextVolumeNumber(series) {
  // Escape regex special characters in series name
  let escaped = series.replace(/[.*+?^${}()|[\\]\\\\]/g, '\\\\$&');
  let pattern = new RegExp('^' + escaped + '.*\\\\bv(\\\\d+)\\\\b.*\\\\.cbz$', 'i');
  let maxVol = 0;
  document.querySelectorAll('.item-chk').forEach(chk => {
    let m = chk.value.match(pattern);
    if (m) {
      let n = parseInt(m[1], 10);
      if (n > maxVol) maxVol = n;
    }
  });
  return maxVol + 1;
}
...
function buildOutname(parsed) {
  let nextVol = getNextVolumeNumber(parsed.series);
  let volStr = "v" + String(nextVol).padStart(2, '0');
  let out = parsed.series + " " + volStr;
  if (parsed.subtitle) out += " - " + parsed.subtitle;
  if (parsed.year) out += " (" + parsed.year + ")";
  return out;
}

function getGroupName(f) { return parseFilename(f).series; }
...
function generateCBZParams(files) {
  ...
  return { outname: buildOutname(parsed) };
}
...
function showSelected() {
  ...
  if (cmdInfo && !cmdInfo.warning) msg += "\\n\\nProposed output name:\\n" + cmdInfo.outname;
  alert(msg);
}
...
function toggleRenameBox() {
  let box = document.getElementById("outnameEdit");
  if (box.style.display === "none" || box.style.display === "") {
    // Cancel the auto-proceed countdown so user can edit freely
    clearTimeout(cbzCountdownTimer);
    clearInterval(cbzCountdownInterval);
    cbzCountdownTimer = null;
    document.getElementById("stopCBZBtn").innerText = "✖ Stop!";
...
function prepareCreateCBZ() {
  ...
  // >6 files warning
  if (files.length > 6) {
    if (!confirm("You selected " + files.length + " files to CBZ. Are you sure?")) return;
  }
  ...
  pendingCBZParams = { absPath, files, outname: cmdInfo.outname };

  // Populate and hide the rename box pre-filled with outname
  let editBox = document.getElementById("outnameEdit");
  editBox.value = cmdInfo.outname;
  editBox.style.display = "none";

  // Build display text
  let displayLines = "Creating: " + cmdInfo.outname + ".cbz\\n";
  displayLines += "From " + files.length + " file(s):\\n";
  files.forEach(f => { displayLines += "  " + f + "\\n"; });
  ...
  document.getElementById("cbzConfirm").style.display = "flex";
  contentDiv.innerText = displayLines;
  ...
  cbzCountdownTimer = setTimeout(() => { proceedCBZ(); }, CBZ_COUNTDOWN_SECS * 1000);
}
...
function stopCBZ() {
  ...
  document.getElementById("outnameEdit").style.display = "none";
...
function proceedCBZ() {
  ...
  // Use edited outname if the rename box is visible and non-empty
  let editBox = document.getElementById("outnameEdit");
  if (editBox.style.display !== "none" && editBox.value.trim()) {
    pendingCBZParams.outname = editBox.value.trim();
  }

  let { absPath, files, outname } = pendingCBZParams;
  pendingCBZParams = null;
  document.getElementById("cbzConfirm").style.display = "none";
  document.getElementById("outnameEdit").style.display = "none";
  ...
  fetch("/", {
    method: "POST", headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "create_cbz_direct", abs_path: absPath, files, outname })
  })
  .then(res => res.json())
  .then(data => {
    contentDiv.innerText += data.output || data.error;
    if (data.success) {
      let reloadDiv = document.getElementById("executionOutput");
      let countdown = 10;
      let refreshMsg = () => {
        contentDiv.innerText = contentDiv.innerText.replace(/\\n--- Refreshing in \\d+ seconds\\.\\.\\. ---/, "");
        contentDiv.innerText += "\\n\\n--- Refreshing in " + countdown + " seconds... ---";
        reloadDiv.scrollTop = reloadDiv.scrollHeight;
...
function checkSubfolders4CBZ() {
  hideWarning();
  lastCsvPath = null; lastViewUrl = null;
  let absPath = document.getElementById("currentAbsPath").value;
  let outputDiv = document.getElementById("executionOutput");
  let contentDiv = document.getElementById("outputContent");
  ...
  .then(data => {
    if (data.error) { contentDiv.innerText += "ERROR: " + data.error; }
      ...
      lastCsvPath = data.csv_path; lastViewUrl = data.view_url;
...
function openCSVDownload() { if (lastCsvPath) window.open(lastCsvPath, '_blank'); }
function openCSVViewer()   { if (lastViewUrl) window.open(lastViewUrl, '_blank'); }
''')
