...
def create_cbz_direct(abs_path: str, files: list[str], outname: str) -> tuple[bool, str]:
    ...
    try:
        ...
        if os.path.exists(working_dir):
            log.append(f'Removing existing working folder: {outname}/')
            shutil.rmtree(working_dir)
        os.makedirs(working_dir)
        log.append(f'Created working folder: {outname}/')
        ...
        for filename in sorted(files):
            ...
            os.makedirs(subdir)
            log.append(f'Extracting: {filename} → {stem}/')
            ...
            if result.returncode != 0:
                ...
                extracted = os.listdir(subdir)
                log.append(f'  → {len(extracted)} file(s) extracted')
        ...
        log.append(f'  zip complete.')

        # Step 4: move CBZ to abs_path
        if os.path.exists(cbz_final):
            os.remove(cbz_final)
        shutil.move(cbz_in_working, cbz_final)
        log.append(f'Moved to: {cbz_final}')

        # Step 5: cleanup working folder
        shutil.rmtree(working_dir)
        log.append(f'Cleaned up working folder.')
        log.append(f'\n✔ Done: {cbz_name}')
        return True, '\n'.join(log)
