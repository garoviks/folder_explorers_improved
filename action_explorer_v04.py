...
Merges v02 (group detection, scroll restore, tooltip, graceful shutdown) and
v03 (rename action, expanded isVolume, pre-declared warning banner, type-hint).
Also fixes the os.chdir thread-safety issue by using subprocess cwd= instead.
"""
...
function getGroupName(filename) {
  ...
  // Leading zero pattern: "Archangel 8 01" -> "Archangel 8"
  let match = name.match(/(?:\\s|-|#|_)+(0\\d+)/i);
  if (match) return name.substring(0, name.indexOf(match[0])).trim();
  // Fallback: first number followed by separator
  match = name.match(/(?:\\s|-|#|_)+(\\d+)(?:[\\.\\s\\-_]+|$)/i);
  if (match) return name.substring(0, name.indexOf(match[0])).trim();
  return name;
}
...
function generateCBZParams(files) {
  ...
  let groupPrefix = Array.from(groups)[0];
  return {
    pattern: groupPrefix + "*.*",
    outname: groupPrefix + " v01"
  };
}
